...
/**
 * Command that removes state records for change files that no longer exist on disk. Only records
 * relevant to the current environment are considered.
 */
public class PurgeOrphanedStateRecordsCommand implements Command<List<StateRecord>> {
    ...
    public List<StateRecord> execute(CommandContext context) {
        ...
        return context.get(StateService.class)
                .purgeOrphanedStateRecords(onDiskIds, environmentName);
    }
}
