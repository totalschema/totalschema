...
│   └── [other dependencies...]
├── user_libs/              # Place JDBC drivers, Groovy, and other user libraries here
│   └── user-libs-README.md
└── lib/jdbc/               # Deprecated (use user_libs/ instead)
    └── lib-jdbc-README.md
```
...
TotalSchema requires additional libraries that cannot be bundled due to licensing or optional features. Place these JAR files in the `user_libs/` directory.

### What Goes in user_libs/?

1. **JDBC Drivers** - For database connectivity
2. **Groovy** - For Groovy-based extensions (groovy-all JAR)
3. **Other Libraries** - Any additional runtime dependencies

### Adding Libraries

```bash
# Example: PostgreSQL driver
cp postgresql-42.7.3.jar totalschema-1.0-SNAPSHOT/user_libs/

# Example: Groovy (for Groovy extensions)
cp groovy-all-2.4.21.jar totalschema-1.0-SNAPSHOT/user_libs/

# Example: Multiple libraries
cp postgresql-42.7.3.jar mysql-connector-j-8.3.0.jar groovy-all-2.4.21.jar totalschema-1.0-SNAPSHOT/user_libs/
```

Multiple libraries can coexist in the same directory. See `user_libs/user-libs-README.md` for detailed documentation.

### Alternative: Using Environment Variable

Alternatively, use the `TOTALSCHEMA_USER_LIBS` environment variable:

```bash
# Linux/macOS
export TOTALSCHEMA_USER_LIBS="/path/to/postgresql.jar:/path/to/groovy-all.jar"

# Windows
set TOTALSCHEMA_USER_LIBS=C:\libs\postgresql.jar;C:\libs\groovy-all.jar
```
...
```bash
# Set user libraries path (JDBC drivers, etc.)
export TOTALSCHEMA_USER_LIBS="/opt/drivers/postgresql-42.7.3.jar:/opt/drivers/ojdbc11.jar"
...
**Solution**: 
1. Verify JDBC driver is in `user_libs/` directory
2. Or set `TOTALSCHEMA_USER_LIBS` environment variable
